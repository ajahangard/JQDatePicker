...
{
    return (persian_to_jd(year + 1, 1, 1) -
        persian_to_jd(year, 1, 1)) > 365;
}
...
function jd_to_persian(jd)
{
    ...
    yday = (Math.floor(jd) - persian_to_jd(year, 1, 1)) + 1;
    month = (yday <= 186) ? Math.ceil(yday / 31) : Math.ceil((yday - 6) / 30);
    day = (Math.floor(jd) - persian_to_jd(year, month, 1)) + 1;

    return new Array(year, month, day);
}

function persian_year(jd) {
